...
RSpec.describe 'Cart Show Page' do
  describe 'As a Visitor' do
    before :each do
      @megan = Merchant.create!(name: 'Megans Marmalades', address: '123 Main St', city: 'Denver', state: 'CO', zip: 80218, discount: 20)
    ...
    describe 'I can manipulate my cart' do
      ...
      it "a 20% discount is applied when I add 20 of the same items to my cart" do
        ...
        expect(page).to have_content("Total: #{ActionController::Base.helpers.number_to_currency((@ogre.price * 20) - ((@ogre.price * 20) * (@ogre.merchant.discount.to_f / 100)))}")
